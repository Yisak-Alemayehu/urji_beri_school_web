...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    if (!isset($_GET['token']) || !verify_csrf($_GET['token'])) {
        ...
        $postCount = $db->fetch("SELECT COUNT(*) as count FROM blog_posts WHERE category_id = ?", [$categoryId])['count'];
        
        if ($postCount > 0) {
            set_flash('error', 'Cannot delete category with existing posts. Please reassign or delete the posts first.');
        } else {
            $db->query("DELETE FROM blog_categories WHERE id = ?", [$categoryId]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            $slug = $editId ? unique_slug($name, 'blog_categories', $editId) : unique_slug($name, 'blog_categories');
            
            try {
                if ($editId) {
                    $db->query("UPDATE blog_categories SET name = ?, slug = ?, description = ? WHERE id = ?", 
                               [$name, $slug, $description, $editId]);
                    set_flash('success', 'Category updated successfully.');
                } else {
                    $db->query("INSERT INTO blog_categories (name, slug, description) VALUES (?, ?, ?)", 
...
if (isset($_GET['edit']) && is_numeric($_GET['edit'])) {
    $editCategory = $db->fetch("SELECT * FROM blog_categories WHERE id = ?", [$_GET['edit']]);
}
        ...
        <div class="admin-card-body">
            ...
            <form method="POST" class="admin-form">
                ...
                <div class="form-actions">
                    ...
                    <?php if ($editCategory): ?>
                        <a href="<?php echo ADMIN_URL; ?>/blog-categories.php" class="btn btn-outline">Cancel</a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
                    ...
                    <tbody>
                        <?php foreach ($categories as $category): ?>
                            <tr>
                                ...
                                <td>
                                    <span class="badge badge-info"><?php echo $category['post_count']; ?> posts</span>
                                </td>
                                <td>
                                    <div class="admin-table-actions">
                                        <a href="<?php echo ADMIN_URL; ?>/blog-categories.php?edit=<?php echo $category['id']; ?>" 
                                        ...
                                        <?php if ($category['post_count'] == 0): ?>
                                            <a href="<?php echo ADMIN_URL; ?>/blog-categories.php?delete=<?php echo $category['id']; ?>&token=<?php echo $_SESSION[CSRF_TOKEN_NAME]; ?>" 
                                        ...
                                        <?php else: ?>
                                            <button class="btn btn-sm btn-outline" disabled title="Cannot delete - has posts" 
